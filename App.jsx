...
export default function App() {
  ...
  const [jugadores,  setJugadores]  = useState({});   // { dni: jugador }
  ...
  // Helpers derivados
  const jugadorPorDni  = (dni) => jugadores[dni];
  ...
  async function guardarJugadores(nuevos) {
    await setDoc(JUGADORES_DOC(), { lista: nuevos });
  }
  async function guardarPartido(nuevoPartido) {
    await setDoc(PARTIDO_DOC(), nuevoPartido);
  }
  async function guardarNotifs(nuevas) {
    await setDoc(NOTIFS_DOC(), { lista: nuevas });
  }
  ...
  async function registrarJugador() {
    ...
    const nuevos = { ...jugadores, [formDni.trim()]: nuevo };
    await guardarJugadores(nuevos);
  ...
  async function inscribirsePartido() {
    ...
    const nuevo = { ...partido, inscriptos: [...(partido.inscriptos || []), dniLogin.trim()] };
    await guardarPartido(nuevo);
  ...
  function autenticarAdmin() {
    if (dniAdminGoles.trim() === partido.adminDni) {
      setAdminAuth(true); setAdminAuthMsg("");
    } else {
      setAdminAuthMsg("DNI incorrecto. Solo el admin puede cargar goles.");
    }
  }
  ...
  async function actualizarGoles(dni, delta) {
    ...
    await guardarPartido({ ...partido, goles: { ...(partido.goles || {}), [dni]: nuevo } });
  }
  ...
  async function finalizarPartido() {
    ...
    const nuevasNotifs = [...notifs, notif];
    await guardarNotifs(nuevasNotifs);
  ...
  async function enviarVotos() {
    ...
    const nuevosVotos = { ...(partido.votos || {}), [votanteBuscado.dni]: votosTemp };
    await guardarPartido({ ...partido, votos: nuevosVotos });
  ...
  async function cerrarVotacion() {
    ...
    (partido.inscriptos || []).forEach(dni => {
      ...
      jugadoresActualizados[dni] = {
        ...
        puntajeTotal: j.puntajeTotal + promedioVotos,
        partidos: j.partidos + 1,
        historial: [...(j.historial || []), {
          fecha: new Date().toLocaleDateString("es-AR"),
          nota: +promedioVotos.toFixed(1),
          goles: golesPartido,
          mvpPartido: dni === mvpPartido,
          mvpEquipo: dni === mvpA || dni === mvpB,
        }],
      };
    });
  ...
  async function generarEquipos() {
    ...
    const eqDni = { equipo1: eq.equipo1.map(j => j.dni), equipo2: eq.equipo2.map(j => j.dni) };
    setEquipos(eq);
    await guardarPartido({ ...partido, equipos: eqDni });
  }
  ...
  const S = {
    ...
    btn:    (v="primary") => ({ padding: "12px 20px", borderRadius: 10, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 14, background: v==="primary"?"#F5A623":v==="danger"?"#E53E3E":v==="green"?"#38A169":v==="admin"?"#805AD5":"#1e3a5f", color: v==="primary"?"#000":"#fff", width: "100%", marginTop: 8, transition: "opacity .2s" }),
        ...
        <div style={S.section}>
          <div style={{ textAlign: "center", padding: "16px 0 20px" }}>
            ...
            <h1 style={{ fontSize: 24, fontWeight: 900, margin: "10px 0 6px" }}>
              Bienvenido a <span style={S.accent}>FútbolApp</span>
            </h1>
            <p style={{ color: "#5a7a99", fontSize: 14, lineHeight: 1.6 }}>
              Todos los datos se guardan en la nube<br/>y se comparten entre todos los jugadores.
            </p>
          </div>
          ...
          <div style={S.card}>
            ...
            <input style={{ ...S.input, marginBottom: 10 }} placeholder="Ej: Juan Pérez"
              value={formNombre} onChange={e => setFormNombre(e.target.value)} />
            <label style={S.label}>DNI (tu ID en la app)</label>
            <input style={S.input} placeholder="Ej: 38123456"
          ...
          {!partido.finalizado ? (
            <>
              <div style={S.card}>
                ...
                <input style={S.input} placeholder="Ingresá tu DNI"
                  value={dniLogin} onChange={e => setDniLogin(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && inscribirsePartido()} />
              ...
              <div style={{ ...S.card, border: "1px solid #805AD544" }}>
                ...
                {adminObj ? (
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 10 }}>
                    ...
                    <button onClick={() => { setAdminMode(true); guardarPartido({ ...partido, adminDni: null }); }}
                      style={{ background: "#1e3a5f", border: "none", color: "#8a9bb0", borderRadius: 8, padding: "6px 10px", cursor: "pointer", fontSize: 12 }}>
                      Cambiar
                    </button>
                  </div>
                ) : (
                  <>
                    ...
                    {!adminMode ? (
                      ...
                      <>
                        ...
                        <input style={S.input} placeholder="DNI del jugador admin"
                          value={dniAdmin} onChange={e => setDniAdmin(e.target.value)} />
                ...
                <div style={{ ...S.card, border: "1px solid #805AD544" }}>
                  ...
                  {!adminAuth ? (
                    <>
                      ...
                      <input style={S.input} placeholder="Ingresá el DNI del admin"
                        value={dniAdminGoles} onChange={e => setDniAdminGoles(e.target.value)} />
              ...
              {inscriptosObj.length > 0 && !partido.adminDni && (
                <div style={{ ...S.card, background: "#1a1a2a", textAlign: "center", color: "#5a7a99", fontSize: 13 }}>
                  ⚠️ Designá un admin para cargar goles y finalizar el partido.
                </div>
              )}
            </>
          ) : (
            <div style={{ ...S.card, textAlign: "center" }}>
              ...
              <div style={{ color: "#5a7a99", fontSize: 13, marginTop: 6 }}>
                Votos: {Object.keys(partido.votos || {}).length} / {inscriptosObj.length}
              </div>
          ...
          {!partido.finalizado ? (
            ...
            <>
              <div style={{ ...S.card, background: "#0d1e10", border: "1px solid #276749" }}>
                <div style={{ fontSize: 13, color: "#68D391" }}>
                  ✅ Votaron {Object.keys(partido.votos || {}).length} / {inscriptosObj.length} jugadores
                </div>
              ...
              {!votanteBuscado ? (
                <div style={S.card}>
                  ...
                  <label style={S.label}>Tu DNI</label>
                  <input style={S.input} placeholder="Ingresá tu DNI"
                    value={dniVotante} onChange={e => setDniVotante(e.target.value)} />
                ...
                <div style={S.card}>
                  ...
                  {inscriptosObj.filter(j => j.dni !== votanteBuscado.dni).map(j => (
                    <div key={j.dni} style={{ marginBottom: 16, paddingBottom: 16, borderBottom: "1px solid #1e3a5f" }}>
                      ...
                      <StarRating value={votosTemp.notas[j.dni] || 0}
                        onChange={v => setVotosTemp(prev => ({ ...prev, notas: { ...prev.notas, [j.dni]: v } }))} />
                    </div>
                  ))}
