...
function PPartido({ comunidad, partido, user, loadComs, setPantalla }) {
  ...
  useEffect(()=>{
    const load = async () => {
      ...
      // Cargar nombres de miembros actuales para filtrarlos
      const nombresMiembros = new Set();
      for(const dni of comunidad.miembros||[]){
        const s = await getDoc(rUser(dni));
        if(s.exists()) nombresMiembros.add(s.data().nombre?.toLowerCase().trim());
      }
      const lista = Object.entries(mapa)
        .map(([nombre,data])=>({nombre,...data}))
        .filter(inv=>!nombresMiembros.has(inv.nombre?.toLowerCase().trim()));
